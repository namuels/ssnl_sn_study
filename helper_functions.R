...
runModelsPrintAll <- function(dataFile, DVList, IVList, categoricalOutcome = FALSE) {
  for(n in 1:length(DVList)) {
    DV = DVList[n]
    IVs = IVList[n]
    IVstring = paste(unlist(IVs), collapse=' + ')
    
    cat(paste(DV, " ~ ", IVstring, sep=""), "\n")
    if(categoricalOutcome) {
      eval(parse(text = paste('thisModel = glm(', DV, ' ~ ', IVstring, ', family=binomial(), dataFile)', sep="")))
    } else {
      eval(parse(text = paste('thisModel = lm(', DV, ' ~ ', IVstring, ', dataFile)', sep="")))
    ...
    for(j in 2:(dim(thisTable)[1])) { # ignore intercept
      #if(thisTable[j,4]<.05) {
        cat(paste("Predictor: ", rownames(thisTable)[j], ", b: ", format(thisTable[j,1], digit=3), ", t: ", format(thisTable[j,3], digits=3), ", p:, ", format(thisTable[j,4], digits=3), sep=""), "\n")
      #}
    }
    #print(summary(thisModel))
    cat("\n")
